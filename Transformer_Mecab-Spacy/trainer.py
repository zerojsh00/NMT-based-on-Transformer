...
class Trainer(object):
    ...
    def __init__(self, optimizer, train_loader, valid_loader, test_loader, n_step, device, save_path,
    ...
    def main(self, model, loss_function, rt_losses=False):
        ...
        for step in range(1, self.n_step+1):
            train_loss, train_acc, n_word_train, n_correct_train = self.train(model, loss_function, step)
            valid_loss, valid_acc, n_word_valid, n_correct_valid = self.valid(model, loss_function)
            test_loss, test_acc, n_word_test, n_correct_test = self.test(model, loss_function)
            ...
            self._print(step, train_loss, valid_loss, test_loss, train_acc, valid_acc, test_acc, n_word_train, n_word_valid, n_word_test, n_correct_train, n_correct_valid, n_correct_test)
            if self.metrics_method == "acc":
                valid_metrics, valid_metrics_list = valid_acc, valid_accs

            elif self.metrics_method == "loss":
                valid_metrics, valid_metrics_list = valid_loss, valid_losses

            lowest_metrics, early_break = self.save_model(model, valid_metrics, valid_metrics_list, lowest_metrics)
            if early_break:
                print(" - early break!!")
                break
        ...
        np.savetxt("trainlog/losses-accs-" + self.save_path.split("/")[-1].split('.')[0] +".txt",
                   np.array([train_losses, train_accs, valid_losses, valid_accs]), fmt="%.4e")
        # Time
        end_time = time.time()
    ...
    def train(self, model, loss_function, step):
        ...
        return train_loss / n_word, train_acc, n_word, n_correct

    def valid(self, model, loss_function):
        """test model"""
        model.eval()
        valid_loss = 0
        n_correct = 0
        ...
        with torch.no_grad():
            for batch in self.valid_loader:
                ...
                n_correct += (pred.argmax(-1) == self.tocpu(real_trg).view(-1)).sum().item()
                n_word += trg.ne(model.pad_idx).sum().item()
                valid_acc = n_correct / n_word

        return valid_loss / n_word, valid_acc , n_word, n_correct
    ...
    def test(self, model, loss_function):
        ...
        return test_loss / n_word, test_acc , n_word, n_correct
    ...
    def _print(self, step, train_loss, valid_loss, test_loss, train_acc,  valid_acc, test_acc, n_word_train, n_word_valid, n_word_test, n_correct_train, n_correct_valid, n_correct_test):
        """print log"""
        print_list = []
        print_list += [f"[{step}/{self.n_step}] Training Result"]
        print_list += [f"  - train_n word: {n_word_train:.4f}\t valid_n word: {n_word_valid:.4f}\t test_n word: {n_word_test:.4f}"]
        print_list += [f"  - train_n correct: {n_correct_train:.4f}\t valid_n correct: {n_correct_valid:.4f}\t test_n correct: {n_correct_test:.4f}"]
        print_list += [f"  - train_acc: {train_acc*100:.4f}%\t valid_acc: {valid_acc*100:.4f}%\t test_acc: {test_acc*100:.4f}%"]
        print_list += [f"  - train_ppl: {np.exp(min(train_loss, 100)):8.4f}\t valid_ppl: {np.exp(min(valid_loss, 100)):8.4f}\t test_ppl: {np.exp(min(test_loss, 100)):8.4f}"]
