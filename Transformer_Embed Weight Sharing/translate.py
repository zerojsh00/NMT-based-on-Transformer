...
if __name__ == "__main__":
    ...
    for num in tqdm(range(len(test))) :
        ...
        # print(rand_data.trg)
        src_sent = " ".join(rand_data.src).strip().replace("  ", " ").replace("▁", "").replace("—","")
        trg_sent = " ".join(rand_data.trg).strip().replace("  ", " ").strip().replace("▁", "").replace("—","")
        pred_sent = " ".join(decode(dec_tensor.squeeze().tolist())).replace("  ", " ").strip().split("<s>")[1][1:].replace("▁", "").replace("—","")
