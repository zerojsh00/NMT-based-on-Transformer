...
class Encoder(nn.Module):
    def __init__(self, src, vocab_len, max_seq_len, n_layer, n_head, d_model, d_k, d_v, d_f,
                 drop_rate=0.1, use_conv=False, pad_idx=1, pos_pad_idx=0):
        super(Encoder, self).__init__()
        self.n_head = n_head
        self.pad_idx = pad_idx
        self.drop_out = nn.Dropout(drop_rate)
        self.embed_layer = Embedding(vocab_len, d_model, src, padding_idx=pad_idx)
...
class Decoder(nn.Module):
    def __init__(self, trg, vocab_len, max_seq_len, n_layer, n_head, d_model, d_k, d_v, d_f,
                 drop_rate=0.1, use_conv=False, pad_idx=1, pos_pad_idx=0):
        super(Decoder, self).__init__()
        self.n_head = n_head
        self.pad_idx = pad_idx
        self.dropout = nn.Dropout(drop_rate)
        self.embed_layer = Embedding(vocab_len, d_model, trg, padding_idx=pad_idx)
        self.pos_layer = PositionalEncoding(max_seq_len+1, d_model, pos_pad_idx=pos_pad_idx)
...
class Transformer(nn.Module):
    """Transformer Model"""
    def __init__(self, src, trg, enc_vocab_len, enc_max_seq_len, dec_vocab_len, dec_max_seq_len,
        ...
        self.encoder = Encoder(src, enc_vocab_len, enc_max_seq_len, n_layer, n_head,
                               d_model, d_k, d_v, d_f,
                               pad_idx=pad_idx,
                               pos_pad_idx=pos_pad_idx,
                               drop_rate=drop_rate,
                               use_conv=use_conv)
        self.decoder = Decoder(trg, dec_vocab_len, dec_max_seq_len, n_layer, n_head,
                               d_model, d_k, d_v, d_f,
                               pad_idx=pad_idx,
                               pos_pad_idx=pos_pad_idx,
                               drop_rate=drop_rate,
